...
public class Case2RichFlatMap extends RichFlatMapFunction<Tuple2<String, String>, Case2Record>{
    ...
    @Override
    public void flatMap(Tuple2<String, String> input, Collector<Case2Record> out) throws Exception {
        // 기 처리된 키에대해서 반환
        ...
        if (list.size() >= 2 ) {
            ...
            // 키 매칭은 없지만 특정 케이스에 맞는 데이터의 경우
        } else if(list.size() == 1) {
            String outList1[] =  list.get(0).split(",");
            if (outList1[0].equals("계좌") && !outList1[4].equals("세이프박스")) {
                Case2Record TempRecord = new Case2Record();
