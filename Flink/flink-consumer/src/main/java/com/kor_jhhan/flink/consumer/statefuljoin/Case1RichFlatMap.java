...
public class Case1RichFlatMap extends RichFlatMapFunction<Tuple2<String, String>, Case1Record>{
    ...
    @Override
    public void flatMap(Tuple2<String, String> input, Collector<Case1Record> out) throws Exception {
        // 기 처리된 키에대해서 반환
        ...
        // 키 매칭되면 추출하고 상태를 초기화
        if (list.size() >= 2 ) {
            Case1Record TempRecord = new Case1Record();
            String outList1[] =  list.get(0).split(",");
            String outList2[] =  list.get(1).split(",");

            // CustomerRecord 담아서 추출
            if(outList1[0].equals("고객")){
                TempRecord.setcol1(outList1[1]);
                TempRecord.setcol2(outList1[3]);
                TempRecord.setcol3((outList1.length >= 5) ? outList1[4] : "");
                TempRecord.setcol4(outList2[3]);
                TempRecord.setcol5(outList2[4]);
            }else{
                TempRecord.setcol1(outList2[1]);
                TempRecord.setcol2(outList2[3]);
                TempRecord.setcol3((outList2.length >= 5) ? outList2[4] : null);
                TempRecord.setcol4(outList1[3]);
                TempRecord.setcol5(outList1[4]);
            }
